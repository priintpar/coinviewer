...
import urllib, json, csv, sys, time
...
def getUrlFromCryptoCoinName(cryptocoinname):
    return "https://api.coinmarketcap.com/v1/ticker/" + cryptocoinname + "/?convert=EUR"
...
def saveValueHistoryCSV(csvline, csvhistoryfilename):
    #argument 1 einlesen bei punkt splitten und dann string-history.csv nennen und speichern
    with open(csvhistoryfilename, 'a') as csvfile:
        wr = csv.writer(csvfile, quoting=csv.QUOTE_ALL)
        wr.writerow(csvline)

def readCurrentCSVProcessed(csvfilename):
    tmplist = csvfilename.split("-")
    tmplist2 = tmplist[1].split(".")
    return tmplist2[0]

def createCurrentCSVHistoryName(currentcsvprocessed):
    return "history-" + currentcsvprocessed + ".csv"
...
if __name__ == '__main__':
    ...
    csvhistoryfilename = createCurrentCSVHistoryName(readCurrentCSVProcessed(currentcsvfilename))
    coinlist = getCryptoCoinsFromCSVFile(currentcsvfilename)
    saveValueHistoryCSV(createValueHistoryCSVLine(coinlist),csvhistoryfilename)
